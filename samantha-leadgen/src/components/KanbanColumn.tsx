...
export default function KanbanColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`min-w-0 ${
        isDragOver ? classes.dragOver : `${classes.bg} ${classes.border}`
      } border-2 border-dashed rounded-lg h-full transition-all duration-200`}
    >
      <div
        className={`${classes.header} px-4 py-3 border-b ${classes.border} rounded-t-lg`}
      >
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-900">{title}</h2>
          <span
            className={`${classes.badge} text-white text-xs px-2 py-1 rounded-full`}
          >
            {count}
          </span>
        </div>
      ...
      <div className="p-4 space-y-3 max-h-[calc(100vh-16rem)] overflow-y-auto overflow-x-hidden">
        {leads.length === 0 ? (
